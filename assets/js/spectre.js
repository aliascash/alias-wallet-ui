...
var blockExplorerPage = {
  ...
  findBlock : function(value) {
    ...
    if ("" === value || null === value) {
      blockExplorerPage.updateLatestBlocks();
        ...
        bridge.findBlockResult.connect(findBlockResult);
        bridge.findBlock(value);
        function findBlockResult(result) {
            console.log("findBlockResult : function(result)");
            bridge.findBlockResult.disconnect(findBlockResult);
  ...
  updateLatestBlocks : function() {
    blockExplorerPage.latestBlocks = bridge.listLatestBlocks();
      bridge.listLatestBlocksResult.connect(listLatestBlocksResult);
      function listLatestBlocksResult(result) {
          bridge.listLatestBlocksResult.disconnect(listLatestBlocksResult);
  ...
  prepareBlockTable : function() {
    $("#latest-blocks-table  > tbody tr").selection().on("click", function() {
      ...
      blockExplorerPage.blkTxns = bridge.listTransactionsForBlock(r20);
        bridge.listTransactionsForBlockResult.connect(listTransactionsForBlockResult);
        function listTransactionsForBlockResult(result) {
            bridge.listTransactionsForBlockResult.disconnect(listTransactionsForBlockResult);
            ...
            $("#block-txs-table > tbody tr").selection().on("dblclick", function(dataAndEvents) {
              ...
              selectedTxn = bridge.txnDetails(r20, $(this).attr("data-value").trim());
                bridge.txnDetailsResult.connect(txnDetailsResult);
                function txnDetailsResult(result) {
                    bridge.txnDetailsResult.disconnect(txnDetailsResult);
                    selectedTxn = result;
      ...
      selectedBlock = bridge.blockDetails($(this).attr("data-value").trim());
        bridge.blockDetailsResult.connect(blockDetailsResult);
        function blockDetailsResult(result) {
            bridge.blockDetailsResult.disconnect(blockDetailsResult);
